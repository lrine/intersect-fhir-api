...
INSTRUCTIONS:
1. Replace Task with the actual resource name (e.g., Practitioner, Device)
2. Replace task with lowercase version (e.g., practitioner, device)
3. Update search parameters based on FHIR spec for that resource
4. Import: from fhir.resources.task import Task
"""
...
async def get_task(
    ...
    """Retrieve a Task resource by ID"""
    db = get_database()
    
    task = await db.Task.find_one({"id": task_id}, {"_id": 0})
